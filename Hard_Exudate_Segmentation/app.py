...
def predict_and_evaluate(image, gt_mask):
    """Predict mask and evaluate against ground truth"""
    # Resize both to match input size
    image_bgr = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
    image_resized = cv2.resize(image_bgr, (W, H))
...
with gr.Blocks() as demo:
    ...
    with gr.Tab("1️⃣ Segment Image Only"):
        with gr.Row():
            image_input = gr.Image(type="numpy", label="Upload Image")
        with gr.Row():
            output_mask = gr.Image(label="Predicted Mask")
        with gr.Row():
            btn1 = gr.Button("Segment")
            btn1.click(fn=predict_mask, inputs=image_input, outputs=output_mask)

    with gr.Tab("2️⃣ Segment + Evaluate with Ground Truth"):
        with gr.Row():
            img_input2 = gr.Image(type="numpy", label="Upload Image")
            gt_input2 = gr.Image(type="numpy", label="Upload Ground Truth Mask (Binary)", image_mode='L')
        with gr.Row():
            composite_output = gr.Image(label="Composite: Input | GT Mask | Prediction")
        with gr.Row():
            metrics_output = gr.Label(label="Evaluation Metrics")
        with gr.Row():
            btn2 = gr.Button("Segment and Evaluate")
            btn2.click(fn=predict_and_evaluate, inputs=[img_input2, gt_input2], outputs=[composite_output, metrics_output])
