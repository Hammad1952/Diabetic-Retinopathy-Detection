...
import numpy as np
import cv2
from model import build_unet  # Ensure this matches your actual model import
...
# Load Soft Exudate Segmentation Model
model = build_unet()
model.load_state_dict(torch.load("files/checkpoint.pth", map_location=DEVICE))  # Update if needed
model.to(DEVICE)
model.eval()

def predict_mask(image):
    """Predict soft exudate mask from fundus image."""
...
def evaluate_metrics(pred, gt):
    ...
    return {
        "IoU (Jaccard)": round(jaccard_score(gt_bin, pred_bin), 4),
...
def predict_and_evaluate(image, gt_mask):
    """Predict + evaluate with ground truth mask for soft exudates."""
    image_bgr = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
    image_resized = cv2.resize(image_bgr, (W, H))
...
with gr.Blocks() as demo:
    gr.Markdown("# 🌟 Soft Exudate Segmentation System")

    with gr.Tab("🖼 Predict Mask Only"):
        with gr.Row():
            img_input = gr.Image(type="numpy", label="Upload Fundus Image")
        with gr.Row():
            mask_output = gr.Image(label="Predicted Soft Exudate Mask")
        with gr.Row():
            btn1 = gr.Button("▶️ Predict Mask")
