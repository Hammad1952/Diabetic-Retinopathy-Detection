...
# Load the model
model = build_unet()
model.load_state_dict(torch.load("files/checkpoint.pth", map_location=DEVICE))  # update path if needed
model.to(DEVICE)
model.eval()

def predict_mask(image):
    """Predict only mask from input image."""
...
def predict_and_evaluate(image, gt_mask):
    """Predict + Evaluate with Ground Truth"""
    image_bgr = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
    image_resized = cv2.resize(image_bgr, (W, H))
...
with gr.Blocks() as demo:
    gr.Markdown("## Optic Disc Segmentation")

    with gr.Tab("🖼️ Predict Mask Only"):
        with gr.Row():
            img_input = gr.Image(type="numpy", label="Upload Fundus Image")
        with gr.Row():
            mask_output = gr.Image(label="Predicted Optic Disc Mask")
        with gr.Row():
